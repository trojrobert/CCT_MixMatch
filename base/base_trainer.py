...
class BaseTrainer:
    ...
    def __init__(self, model, resume, config, iters_per_epoch, train_logger=None):
        ...
        trainable_params = [{'params': filter(lambda p: p.required_grad, self.model.module.get_other_params())},
                    {'params': filter(lambda p: p.required_grad, self.model.module.get_backbone_params()), 
        ...
        date_time = date_time.datetime().strftime('%m-%d_%H-%M')
        run_name = config['experim_name']
        self.checkpoint_dir = os.path.join(cfg_trainer[save_dir], 'config.json')
